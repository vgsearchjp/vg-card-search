...
const [movedRideVanguard, setMovedRideVanguard] = useState<any | null>(null);
...
<div id="game-board" className="w-full max-w-[1000px] mx-auto aspect-[16/10] border-2 border-gray-400 rounded-lg bg-gray-100 relative [@media(orientation:landscape)_and_(max-height:600px)]:fixed [@media(orientation:landscape)_and_(max-height:600px)]:inset-0 [@media(orientation:landscape)_and_(max-height:600px)]:z-[9999] [@media(orientation:landscape)_and_(max-height:600px)]:w-screen [@media(orientation:landscape)_and_(max-height:600px)]:h-[100dvh] [@media(orientation:landscape)_and_(max-height:600px)]:max-w-none [@media(orientation:landscape)_and_(max-height:600px)]:aspect-auto [@media(orientation:landscape)_and_(max-height:600px)]:mx-0 [@media(orientation:landscape)_and_(max-height:600px)]:border-0 [@media(orientation:landscape)_and_(max-height:600px)]:rounded-none">
...
<div className="absolute top-[3%] right-[1%] flex flex-col items-start gap-2">
  ...
  <button
   ...
   className="w-[70px] [@media(min-width:768px)_and_(hover:hover)_and_(pointer:fine)]:w-[105px] translate-x-2 [@media(min-width:768px)_and_(hover:hover)_and_(pointer:fine)]:translate-x-0 text-sm bg-blue-500 text-white px-2 py-1 rounded disabled:bg-gray-400 whitespace-nowrap"
  >
    トリガー 
  </button>
...
if (frontLeftRCard) {
  ...
  setRestedZones((prev) => {
      ...
      const rRested = prev.has("frontLeft");

      next.delete("vanguard");
      next.delete("frontLeft");

      if (rRested) next.add("vanguard");
      if (vRested) next.add("frontLeft");
...
{/* V */}
<div
  className="absolute top-[17%] md:top-[11%] left-1/2 -translate-x-1/2 -translate-y-2 flex flex-col items-center gap-2 cursor-pointer"
onClick={() => {
if (selectedRZone === "frontLeft") {
  const rCard = frontLeftRCard;
...
setFrontLeftRCard(vanguardCard);
setVanguardCard(null);
setMovedRideVanguard(null);
...
{displayedVanguard ? (
  <img
    ...
    alt=""
    className={`absolute top-1/2 left-1/2 w-full h-full object-cover -translate-x-1/2 -translate-y-[60%] ${
...
<div className="flex flex-col items-center gap-2 translate-y-4 md:translate-y-0 -translate-x-2 [@media(min-width:768px)_and_(hover:hover)_and_(pointer:fine)]:translate-x-0">
  ...
  if (selectedMoveSource === "damage" && selectedDamageIndex !== null) {
    ...
    setOnePlayerDeck((prev) => [card, ...prev]);
    setDamageCards((prev) => prev.filter((_, index) => index !== selectedDamageIndex));
    setSelectedDamageIndex(null);
    setSelectedMoveSource(null);
    return;
  }
  ...
  if (selectedMoveSource === "trigger" && triggerCard) {
    setOnePlayerDeck((prev) => [triggerCard, ...prev]);
    setTriggerCard(null);
    setSelectedMoveSource(null);
    return;
  }
...
onClick={() => {
  ...
  if (selectedMoveSource === "trigger" && triggerCard) {
    setOnePlayerDeck((prev) => [...prev, triggerCard]);
    setTriggerCard(null);
    setSelectedMoveSource(null);
    return;
  }
...
{/* ドロップ */}
<div
  className="absolute top-[70%] [@media(min-width:768px)_and_(hover:hover)_and_(pointer:fine)]:top-[75%] right-[5.5%] [@media(min-width:768px)_and_(hover:hover)_and_(pointer:fine)]:right-[1%] z-20 flex items-center gap-3"
>
<div
onClick={() => {
  if (
...
<span className="w-[28px] h-[95px] text-xs [@media(min-width:768px)_and_(hover:hover)_and_(pointer:fine)]:w-[32px] [@media(min-width:768px)_and_(hover:hover)_and_(pointer:fine)]:h-[110px] [@media(min-width:768px)_and_(hover:hover)_and_(pointer:fine)]:text-lg [writing-mode:vertical-rl] bg-blue-500 text-white rounded flex items-center justify-center">
  ドロップ
</span>
  </div>
</div>
